...
class FSDPStrategy(TrainingStrategy):
    def __init__(
        ...
        self.fsdp_optimizer_save_policy = FullOptimStateDictConfig(offload_to_cpu=True, rank0_only=True)
    ...
    def save_checkpoint(
        ...
        only_trainable = not self.save_vision_backbone
        ...
        # Save optimizer, and scheduler state dicts
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy, self.fsdp_optimizer_save_policy):
                optim_state = FSDP.optim_state_dict(self.vlm, self.optimizer)
                # Save on rank zero *only*
                if overwatch.is_rank_zero():
                    checkpoint_dir = run_dir / "checkpoints"
                    optimizer_checkpoint_path = (
                        checkpoint_dir / f"optimizer-{global_step:06d}-epoch-{epoch:02d}.pt"
                    )

                    # Save Optimizer & Copy Latest to `latest-optimizer.pt`
                    torch.save(optim_state, optimizer_checkpoint_path)
                    shutil.copy(optimizer_checkpoint_path, checkpoint_dir / "latest-optimizer.pt")
                    
                    scheduler_checkpoint_path = (
                        checkpoint_dir / f"scheduler-{global_step:06d}-epoch-{epoch:02d}.pt"
                    )

                    # Save Scheduler & Copy Latest to `latest-optimizer.pt`
                    torch.save(self.lr_scheduler.state_dict(), scheduler_checkpoint_path)
                    shutil.copy(scheduler_checkpoint_path, checkpoint_dir / "latest-scheduler.pt")


    def load_latest_checkpoint(self, run_dir: Path) -> Optional[int]:
        """
        Load the latest checkpoint to resume training.
        
        Args:
            run_dir (Path): The directory containing the checkpoints.
        
        Returns:
            Optional[int]: The global step to resume from, or None if no checkpoint is found.
        """
        checkpoint_dir = run_dir / "checkpoints"
        latest_checkpoint_path = checkpoint_dir / "latest-checkpoint.pt"
        latest_optimizer_path = checkpoint_dir / "latest-optimizer.pt"
        scheduler_checkpoint_path = checkpoint_dir / "latest-scheduler.pt"
        optim_state = None

        if not latest_optimizer_path.exists():
            overwatch.info("No checkpoint found to resume training.")
            return None
        else:
            with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy, self.fsdp_optimizer_save_policy):
                if overwatch.is_rank_zero():
                    optim_state = torch.load(latest_optimizer_path)
        # flattened_osd = FSDP.optim_state_dict_to_load(model=self.vlm, optim=self.optimizer, optim_state_dict=optim_state)
        # self.optimizer.load_state_dict(flattened_osd)

        sharded_osd = FSDP.scatter_full_optim_state_dict(optim_state, self.vlm)
        ...
        for ckpt in checkpoint_dir.iterdir():
            if str(ckpt.stem).startswith("step"):
                step_num = int(str(ckpt.stem).split("-")[1])
                if step_num > global_step:
                    global_step = step_num
        
        overwatch.info(f"Resumed training from global step {global_step}.")
        
        return global_step


    def run_setup(self, run_dir: Path, n_train_examples: int, save_vision_backbone: bool) -> None:
        self.save_vision_backbone = save_vision_backbone
