...
from transformers import LlamaTokenizerFast, Phi3ForCausalLM
...
class HFCausalLLMBackbone(LLMBackbone, ABC):
    def __init__(
        ...
        if not self.inference_mode:
            ...
            # reading with all GPU nodes requires high memory
            import psutil
            mem = psutil.virtual_memory()[0] // 2**30
            if mem < 400:
                local_rank = int(os.environ["LOCAL_RANK"])
                import time
                time.sleep(20 * local_rank)
                self.llm = llm_cls.from_pretrained(
                    os.path.join(mount_path, hf_hub_path),
                    # token=hf_token,
        ...
        # [Contract] `inference_mode` means we're loading from a pretrained checkpoint; no need to load base weights!
            ...
            # if "phi-3" in hf_hub_path.lower():
            #     self.llm = Phi3ForCausalLM.from_pretrained(os.path.join(mount_path, hf_hub_path))
            # else:
            llm_config = AutoConfig.from_pretrained(os.path.join(mount_path, hf_hub_path))
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(os.path.join(mount_path, hf_hub_path), model_max_length=self.llm_max_length)#, token=hf_token)

        # add this to be compatible with Mistral-7b-v0.1, Phi-3-mini
        if isinstance(self.tokenizer, LlamaTokenizerFast) and self.tokenizer.padding_side != "right":
