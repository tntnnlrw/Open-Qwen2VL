...
class ModelConfig(ChoiceRegistry):
    ...
    # Pre-Train Stage Optimization Parameters
    pretrain_epochs: int                                    # Epochs to Run (in case `max_steps` is not specified)
    pretrain_max_steps: Optional[int]                       # [Optional] Max Gradient Steps (overrides epochs)
    pretrain_global_batch_size: int                         # Global Batch Size (divided across processes)
    pretrain_per_device_batch_size: int                     # Per-Device Batch Size (per-process)
                                                            #   => # of accumulation steps is auto-computed

    pretrain_learning_rate: float                           # Peak Learning Rate (lr_scheduler sets warmup/decay)
    pretrain_weight_decay: float                            # Weight Decay for AdamW Optimizer
    pretrain_max_grad_norm: float                           # Max Grad Norm (for global gradient clipping)
    pretrain_lr_scheduler_type: str                         # LR Scheduler (default: "linear-warmup+cosine-decay")
    pretrain_warmup_ratio: float                            # Fraction of total steps to warmup

    pretrain_train_strategy: str                            # Finetune Train Strategy (default: "fsdp-full-shard")
...
class LLaVa_v15_Reproduction_7B(ModelConfig):
    ...
    llm_backbone_id: str = "llama2-7b-pure" # change it to llama2 7b
    ...
    # Pre-Train Stage Optimization Parameters
    pretrain_epochs: int = 1                                       # Epochs to Run (in case `max_steps` is not specified)
    pretrain_max_steps: Optional[int] = None                       # [Optional] Max Gradient Steps (overrides epochs)
    pretrain_global_batch_size: int = 512                          # Global Batch Size (divided across processes)
    pretrain_per_device_batch_size: int = 16                       # Per-Device Batch Size (per-process)

    pretrain_learning_rate: float = 5e-5                           # Peak Learning Rate (lr_scheduler sets warmup/decay)
    pretrain_weight_decay: float = 0.01                            # Weight Decay for AdamW Optimizer
    pretrain_max_grad_norm: float = 1.0                            # Max Grad Norm (for global gradient clipping)
    pretrain_lr_scheduler_type: str = "linear-warmup+cosine-decay" # LR Scheduler (default: "linear-warmup+cosine-decay")
    pretrain_warmup_ratio: float = 0.03                            # Fraction of total steps to warmup

    pretrain_train_strategy: str = "fsdp-full-shard"               # Pretrain Train Strategy (default: "fsdp-full-shard")
...
@dataclass
class LLaVa_v15_Reproduction_8B(LLaVa_v15_Reproduction_7B):
    model_id: str = "reproduction-llava-v15+8b"
    arch_specifier: str = "full-align+gelu-mlp"
...
class ModelRegistry(Enum):
    ...
    REPRODUCTION_8B = LLaVa_v15_Reproduction_8B
