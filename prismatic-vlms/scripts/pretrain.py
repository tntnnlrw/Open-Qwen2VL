...
from prismatic.preprocessing import get_dataset_and_collator, get_wds_datainfo
...
class PretrainConfig:
    ...
    # ---
    stage: str = "pretrain"                                         # Pretraining Stage in < align | finetune >
    ...
    mount_path: Union[str, Path] = Path("/path/to/models")          # Path to models

    # Tracking Parameters
    trackers: Tuple[str, ...] = ("jsonl", "wandb")                  # Trackers to initialize (if W&B, add config!)
    wandb_project: str = "mmpretrain"                               # Name of W&B project (default: `mmpretrain`)
    wandb_entity: Optional[str] = None                              # Name of W&B entity (default: None)

    # deal with cpu limit
    # low_cpu_fsdp: Optional[str] = None
    ...
    def __post_init__(self) -> None:
        ...
        elif self.stage.endswith("pretrain"):
            self.epochs = self.model.pretrain_epochs
            self.max_steps = self.model.pretrain_max_steps
            self.global_batch_size = self.model.pretrain_global_batch_size
            self.per_device_batch_size = self.model.pretrain_per_device_batch_size

            self.learning_rate = self.model.pretrain_learning_rate
            self.weight_decay = self.model.pretrain_weight_decay
            self.max_grad_norm = self.model.pretrain_max_grad_norm
            self.lr_scheduler_type = self.model.pretrain_lr_scheduler_type
            self.warmup_ratio = self.model.pretrain_warmup_ratio

            self.train_strategy = self.model.pretrain_train_strategy
...
def pretrain(cfg: PretrainConfig) -> None:
    ...
    llm_backbone, tokenizer = get_llm_backbone_and_tokenizer(
        cfg.model.llm_backbone_id, llm_max_length=cfg.model.llm_max_length, mount_path=mount_path
    )
